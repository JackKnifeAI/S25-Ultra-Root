...
#define SU_TCP_PORT 18899  /* TCP fallback for SELinux Enforcing bypass */
...
static int connect_daemon(void) {
  /* Try UNIX socket first (SO_PEERCRED works here) */
...
static int daemon_main(void) {
  ...
  /* Listen on UNIX socket (backward compat, works with Permissive) */
  int unix_fd = socket(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if (unix_fd >= 0) {
    struct sockaddr_un sun;
    memset(&sun, 0, sizeof(sun));
    sun.sun_family = AF_UNIX;
    ...
    snprintf(sun.sun_path, sizeof(sun.sun_path), "%s", BOOTSTRAP_SOCK_PATH);
    if (bind(unix_fd, (struct sockaddr *)&sun, sizeof(sun)) != 0 ||
        listen(unix_fd, 16) != 0) {
      close(unix_fd);
      unix_fd = -1;
    } else {
  ...
  /* Listen on TCP localhost (SELinux Enforcing bypass) */
  int tcp_fd = socket(AF_INET, SOCK_STREAM | SOCK_CLOEXEC, 0);
  if (tcp_fd >= 0) {
    int reuse = 1;
    ...
    sin.sin_port = htons(SU_TCP_PORT);
  ...
  int maxfd = (unix_fd > tcp_fd) ? unix_fd : tcp_fd;

  for (;;) {
    fd_set rfds;
    FD_ZERO(&rfds);
    if (unix_fd >= 0) FD_SET(unix_fd, &rfds);
    ...
    int listen_fd = -1;
    if (unix_fd >= 0 && FD_ISSET(unix_fd, &rfds)) listen_fd = unix_fd;
    else if (tcp_fd >= 0 && FD_ISSET(tcp_fd, &rfds)) listen_fd = tcp_fd;
