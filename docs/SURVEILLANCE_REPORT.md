...
## SIM Processor / UICC — Carrier Remote Control (Deep Dive)

### What We Found
The SIM processor (UICC) is a SEPARATE computer inside your phone that the carrier controls remotely. Through the SIM Toolkit (STK) protocol, the carrier can:

1. **Open data channels** (BIP — Bearer Independent Protocol) that bypass Android entirely
2. **Send SMS silently** without user knowledge (STKSendSMSforDualBearer)
3. **Execute APDU commands** remotely (DoOemRmtUimApdu)
4. **Refresh/reprogram the SIM** remotely (SimRefreshCmd, REFRESHRESET)
5. **Lock/unlock the device** remotely (DoOemUiccSimLockDeperso)
6. **Track location events** via STK event subscriptions

### Active STK Event Subscriptions (from gsm.sim.eventList: 0x1550)
- **Location Status** (bit 4) — SIM receives location change notifications
- **Idle Screen Available** (bit 6) — SIM knows when you're looking at your phone
- **Data Available** (bit 8) — SIM notified when data channel is ready
- **Channel Status** (bit 10) — SIM monitors BIP channel state
- **Network Search Mode Change** (bit 12) — SIM tracks network changes

### Samsung RIL STK Infrastructure
```
DoStkEnvelopecmd     — Carrier sends commands to phone
DoSendTerminalResponse — Phone executes carrier commands
DoStkUserCnfrm       — User confirmation (can be suppressed)
DoOemRmtUimApdu      — Remote APDU to SIM processor
DoOemUiccRecovery    — Remote SIM recovery
bip_start_notification — SIM opens own data channel
isim_refresh         — Remote SIM reprogramming
```

### Bell Canada (302610) Carrier Identity
- Operator: Bell
- Country: CA (Canada)
- DSDS: Dual SIM Dual Standby configured
- eSIM: Slot 2, MEP mode enabled

### Mitigation: SIM Processor Lockdown
To prevent carrier remote control:
1. Disable STK event subscriptions via RIL command
2. Block BIP data channel creation
3. Disable SIM refresh/OTA provisioning
4. Monitor APDU traffic between AP and SIM
5. Use iptables to block carrier provisioning endpoints
